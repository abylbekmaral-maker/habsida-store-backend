...
3. **Документация API (Swagger UI):**
    После запуска перейти по ссылке: [http://localhost:8080/swagger-ui.html](http://localhost:8080/swagger-ui.html)
...
5. **Аутентификация(JWT)**
   ...
   Выполните POST /api/v1/auth/login с данными администратора (см. ниже).
   Скопируйте полученный accessToken.
...
6. **Предзагруженые тестовые данные**
   ...
   Password: devUser
   Магазин: Fruit Shop (slug: fruit-shop) — привязан к devUser.
   Категории: Apples, Citrus.
